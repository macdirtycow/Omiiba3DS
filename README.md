...
**DS Widescreen Labs workflow:** use **Omiiba Boot Hub** → **DS Widescreen Labs** to check the bundled TWiLight widescreen compatibility list, create `SD:/omiiba/DS_WIDESCREEN_LABS.txt`, create helper folders, and toggle the existing `Enable DSi external filter` setting from the Hub. The active filter path is `SD:/omiiba/twl_upscaling_filter.bin`; many games still need per-game TWiLight widescreen codes/settings.
...
**Wireless Tools workflow:** because the arm9 boot menu is not a safe place to run a full Wi-Fi downloader or FTP server, **Wireless Tools** prepares and checks the standard app paths instead: `SD:/cias/Universal-Updater.cia` and `SD:/3ds/ftpd/ftpd.3dsx`. Use those apps from HOME Menu/Homebrew Launcher for wireless updates and SD file transfer.
...
* Hardcoded version `v1.4.6` (no longer derived from upstream Luma git tags)
...
* **Wireless Tools:** prepares `SD:/cias/` and `SD:/3ds/ftpd/` paths and documents the safe Universal-Updater/ftpd workflow instead of attempting risky bootloader networking
